        ...
        <main class="flex-1 flex flex-col items-center justify-center px-4 py-10 sm:py-14">
            ...
            <div class="text-center mb-6 space-y-1">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                    Masuk untuk mengelola sistem akses ruang server.
                </p>
            </div>
            ...
            <div class="w-full max-w-[420px] bg-white dark:bg-[#060a07] border border-slate-200/80 dark:border-[#193521] rounded-3xl p-6 sm:p-8 shadow-xl shadow-slate-200/40 dark:shadow-none space-y-6">
                ...
                <!-- Session Status (Jika ada notifikasi sukses/status) -->
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-4">
                    ...
                    <!-- Input Nama Pengguna (Ganti name="login" menjadi name="username") -->
                    <div>
                    ...
                    <div class="flex items-center justify-between pt-1">
                        <label for="remember_me" class="inline-flex items-center gap-2 cursor-pointer">
                            <input id="remember_me" 
                                   type="checkbox" 
                                   name="remember" 
                                   class="w-3.5 h-3.5 rounded border-slate-300 dark:border-slate-700 text-[#087f3f] focus:ring-[#087f3f] bg-transparent">
                            <span class="text-[11px] text-slate-500 dark:text-slate-400">Ingat Saya</span>
                        </label>
                ...
                <!-- Tautan ke Halaman Registrasi (Opsional) -->
                @if (Route::has('register'))
                    <div class="pt-2 text-center border-t border-slate-100 dark:border-slate-800/80">
                        <p class="text-xs text-slate-500 dark:text-slate-400">
                            Belum memiliki akun? 
                            <a href="{{ route('register') }}" class="font-bold text-[#087f3f] dark:text-[#16c968] hover:underline">
                                Daftar Akun
                            </a>
                        </p>
                    </div>
